...
String timeAndDate(){
  if (gps.date.isValid() && gps.time.isValid()){
    String result = String (gps.date.month()) + "/" + String (gps.date.day()) + "/" +
        String (gps.date.year()) + " " + String (gps.time.hour() + 8) + ":" + String (gps.time.minute()) + ":" + String (gps.time.second());
        return result;
  } 
  return " INVALID ";
}
...
String full(){
  return (String) longitude() + latitude() + timeAndDate() + distance( longitude() , latitude() ) 
      + bearing( longitude(), latitude() ) + "Turn: " + turn(longitude(), latitude());
}
